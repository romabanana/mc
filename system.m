...
##   dij57_x = (x5-x7) / L57;  dij57_y = (y5-y7) / L57;
  ...
  # F's
   ...
   F13_x = k*(1-L0/L13)*(x3-x1);
   F13_y = k*(1-L0/L13)*(y3-y1);


   F23_x = k*(1-L0/L23)*(x3-x2);
   F23_y = k*(1-L0/L23)*(y3-y2);

   F24_x = k*(1-L0/L24)*(x4-x2);
   F24_y = k*(1-L0/L24)*(y4-y2);

   F34_x = k*(1-L0/L34)*(x4-x3);
   F34_y = k*(1-L0/L34)*(y4-y3);

   F35_x = k*(1-L0/L35)*(x5-x3);
   F35_y = k*(1-L0/L35)*(y5-y3);

   F45_x = k*(1-L0/L45)*(x5-x4);
   F45_y = k*(1-L0/L45)*(y5-y4);

   F46_x = k*(1-L0/L46)*(x6-x4);
   F46_y = k*(1-L0/L46)*(y6-y4);

   F56_x = k*(1-L0/L56)*(x6-x5);
   F56_y = k*(1-L0/L56)*(y6-y5);

   F57_x = k*(1-L0/L57)*(x7-x5);
   F57_y = k*(1-L0/L57)*(y7-y5);

   F67_x = k*(1-L0/L67)*(x7-x6);
   F67_y = k*(1-L0/L67)*(y7-y6);
